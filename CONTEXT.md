...
- **M6 觀點知識庫 pipeline（ORM 化）**：新增 `apps/summary/pipeline/`（`quality_filter.py` Step1-3 純函式、`dedup.py` 用 `pgvector.django.CosineDistance` 取代手寫 SQL、`writer.py` 用 `DialogueSummary.objects.create` / `ViewpointNode.objects.create` 取代手寫 `INSERT`，embedding 改呼叫既有的 `chat.services.embedding.get_embedding`）+ `apps/summary/tests_pipeline.py`。來源是 `feat/polarbear` 分支獨立跑的 `品質篩選.py`/`去重檢查.py`/`觀點寫入.py`/`embedding.py` prototype，邏輯移植、改寫成 Django ORM 後併入 `backend/`，原獨立腳本已移除。**尚未**：串接進對話結束的實際觸發點（無 view/signal/task 呼叫）、逐則 `ccnd_semantic_dist`/`ccnd_stance_shift` 仍無產出來源、本機 `.venv` 套件不全尚未實跑過 `pytest`。
...
| M6 摘要/知識庫 | 🟡 | ✅ 對話後問卷（`PostDialogueResponse`）+ debriefing 同意/撤回 + Part F 平台體驗回饋（`PlatformFeedback`）；✅ 觀點知識庫 ORM pipeline（`apps/summary/pipeline/`，品質篩選/去重/寫入，見待提交變更）；⬜ 立場偏移報告；⬜ pipeline 尚未接進對話結束流程、逐則 `ccnd_semantic_dist`/`ccnd_stance_shift` 無產出來源 |
...
- [ ] M6：立場偏移量化報告
- [ ] M6：觀點知識庫 pipeline 接進對話結束的實際觸發點（`apps/summary/pipeline/` 已有 ORM 版本，缺呼叫點）
- [ ] M6：逐則 `ccnd_semantic_dist` / `ccnd_stance_shift` 產出來源（`apps/summary/pipeline/quality_filter.py` Step2/3 依賴這兩個欄位）
