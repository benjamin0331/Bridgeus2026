...
**例外（語義樹節點生成，2026-06-26 新增）：**
- `apps/matching/services/semantic_tree.py` 的節點生成現依 `topic_id` 分派：
  - `topic_id=102`（核能）：改用本地兩階段 BERT 分類器（`apps/matching/services/nuclear_node_classifier.py` + `apps/matching/ml_models/nuclear_node_model/`），不呼叫 LLM。macro 模型先分類到 6 大錨點之一（或「其他」直接捨棄），再用對應 micro 模型分到 36 個細項其中之一。
  - 其他議題（如 103 女性義務兵役）：維持原本呼叫 OpenAI 的生成式分類（`analyze_with_openai`）。
  - 模型權重（~2.7GB）未進版控，見 `apps/matching/ml_models/nuclear_node_model/README.md`。
