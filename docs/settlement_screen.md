...
## 分頁內容
- **Page 1 — 立場軌跡**：1–7 立場刻度尺（`s_pre`→`s_post`，4=中立）＋ `delta_s` 有向長條 ＋ `stance_centrism` 雙向 meter。
- **Page 2 — 思辨投入**：7 項投入量表**雷達圖** ＋ S/A/B/C 評級印章 ＋ 頭銜。
- **Page 3 — 對話花絮**：`opponent_judgment` 猜對手揭曉（H-AI）＋ `experiment_condition` 對象標籤 ＋ 讚倒讚計數甜甜圈。

## 圖表配色（已過 dataviz 驗證器）
米色紙面 `#f7f3f0` 上：印章 terracotta `#b05540`、綠 `#4c7a3c`、暖灰 `#8a7d6f`、墨 `#2f2722`。
- 驗證器擋掉的：teal `#3f7a86`（讀成灰，棄用）、綠↔terracotta 色盲太近（甜甜圈改綠 vs 暖灰＋👍👎 圖示與數字，識別不靠顏色）、gold 對比不足（不當填色）。
