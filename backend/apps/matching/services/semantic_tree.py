    ...
    *,
    ...
    prompt_text = "\n".join(
        [
            ...
            "- 如果輸入太模糊，輸出 items: []。",
            "",
            "stance 立場判斷與分類規則（客觀嚴謹判斷真實與潛在立場）：",
            "- stance 只能輸出：支持、反對、無關、中立 四者之一。",
            "- 判斷步驟：",
            "  1. 相關性檢驗：若完全離題、純屬無針對議題的情緒發洩、無意義語助詞，直接標記為【無關】。",
            "  2. 表面內容理解：先理解輸入文字表面上直接陳述的內容。",
            "  3. 語言線索與真實態度檢驗（潛在立場）：",
            "     * 反諷：用誇張正面詞彙描述負面情況（如「真是太『棒』了」搭配負面情境）。",
            "     * 反問句：以疑問句表達強烈態度（如「這樣真的對嗎？」），需依句中用詞與語氣綜合判斷立場，若無法判斷則以字面為準。",
            "     * 預設前提：話中有話或隱含的價值判斷（如「連小孩都看得出來的問題，怎麼還在討論？」隱含負面立場）。",
            "  4. 判定依據：只有在找到明確語言線索時，才以「真實意圖（潛在立場）」作為標籤；否則一律以字面為準，避免過度腦補，",
            "- 分類定義：",
            "  * 【支持】：明確或隱含地認同該節點主張、正面評價、提供支持論據或贊成態度。",
            "  * 【反對】：明確或隱含地質疑、反駁、批判該節點主張、提供反方論據、負面評價，或以反諷/反問表達不滿。",
            "  * 【中立】：與議題直接相關，但純粹客觀陳述事實、雙方論點並陳，且無明顯傾向。",
            "  * 【無關】：與目標議題完全離題、純情緒發洩或無意義語助詞。",
            "",
            "輸出品質要求：",
            "- 請避免製造太多節點。",
            "- 請避免同義節點。",
    ...
    *,
    ...
    prompt_text = "\n".join(
        [
            ...
            "",
            "### 判斷步驟：",
            "1. 先判斷【使用者輸入】是否與目標議題節點直接相關。",
            "   - 若完全無關、純屬情緒發洩（未針對議題本身）、無意義語助詞，或無法判斷主旨，直接歸類為【無關】，不需執行後續步驟。",
            "2. 若與議題相關，先理解句子表面上直接陳述的內容。",
            "3. 再檢查是否存在明確的語言線索，顯示表面文字與說話者的真實態度不一致，例如：",
            "   - 反諷：用誇張、不合常理的正面詞彙描述明顯負面的情況（如「真是太『棒』了」搭配負面情境）。",
            "   - 反問句：以疑問句式表達強烈態度，而非單純提問（如「這樣真的對嗎？」）。",
            "     反問句的立場方向，需依句中的正負面用詞、語氣強度、與上下文脈絡綜合判斷，不可一律歸類為同一立場；",
            "     若反問句的方向無法從文字線索判斷，則以字面內容為準。",
            "   - 話中有話、預設前提中隱含的價值判斷（如「連小孩都看得出來的問題，怎麼還在討論？」隱含負面立場）。",
            "4. 只有在步驟 3 找到明確語言線索時，才以「潛在立場（真實意圖）」推翻字面意思作為最終標籤；",
            "   若找不到足夠線索，一律以字面內容為準，避免過度推論、腦補弦外之音。",
            "5. 若句子與議題相關、但字面與語氣皆未表現出明顯傾向（如僅客觀陳述事實、正反論點並陳），則歸類為【中立】。",
            "",
            "### 分類定義：",
            "1. 【支持】：明確或（依步驟3-4判定的）隱含地認同該主題的主張、正面評價、提供支持論據，或表達贊成態度。",
            "2. 【反對】：明確或（依步驟3-4判定的）隱含地質疑、反駁、批判該主題的主張、提供反方論據、負面評價，或使用反諷/反問表達不滿。",
            "3. 【中立】：與目標議題直接相關，但僅客觀陳述事實、雙方論點並陳，且無法從語氣中推斷出潛在立場傾向。",
            "4. 【無關】：與目標議題節點完全離題、純屬情緒發洩（未針對議題）、無意義語助詞，或無法判斷主旨。",
            "",
            "### 輸出限制：",
            "- 請直接輸出「支持」、「反對」、「無關」、「中立」其中一個標籤。",
