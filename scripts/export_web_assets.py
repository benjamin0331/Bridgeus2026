...
前端沒辦法讀 res:// 的資源，所以等級卡上的青蛙、虛擬大廳角落的動畫青蛙都要先在這裡
產好。改了 recolor_frog.py 的配色之後要重跑這支。
...
產出：
    ...
    emoji0.png ~ emoji4.png  虛擬大廳卡片上散落的表情裝飾（見 ScatterDecor.jsx）。座標
                             刻意跟 Globals/Emoji.gd 的 REGIONS 一致，所以網頁上看到的
                             就是玩家在遊戲裡按的那 5 個

卡片背景現在是 map_tape.png，由 scripts/gen_map_tape.py 產生（膠帶拼貼），不在這裡。
更早的 map_bg.png 是人工從遊戲場景截的圖，留著當那張拼貼的底稿。曾經寫過腳本解析
Game.tscn 重畫地圖，但水域會變成大片深褐色方塊，所以放棄了（見
docs/godot_level_colors.md §六之二）。
"""
